...
```bash
mongosh < init_schema.js
```

3. Seed default data:
```bash
mongosh < seed_data.js
```

4. (Optional) Import sample articles:
```bash
mongoimport --db insightbot --collection articles --file articles_sample.json --jsonArray
```
## ⚡ Prefect Pipeline
...
- *user_preferences* → username, filters  
- *app_store* → key, value  

Indexes are defined in init_schema.js.
...
### ✅ User Dashboard
- Browse articles with:
  - Date range filter  
  - Language filter (English, Arabic, Russian, All)  
...
- Maintenance: recompute `time_spent_minutes` from logs.  
...
- *Preferences*: starter filter for admin.  
